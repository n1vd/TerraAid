...
import { AlertTriangle, BarChart3, Bell, CalendarDays, CheckCircle2, Crosshair, Database, FileText, Info, Layers3, Leaf, Map as MapIcon, MapPin, RefreshCw, Satellite, Search, ShieldCheck, SlidersHorizontal, Sparkles, XCircle } from 'lucide-react';
...
const colorFor = (farm: Farm) => farm.Potentially_Missed ? '#b385ff' : severity(farm)==='Severe' ? '#ff6b65' : severity(farm)==='Moderate' ? '#eeb84f' : '#51d49a';
...
function SatelliteMap({ farms, selected, onSelect, date, location, layers, onStatus }: {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const flood = L.polygon([[15.168,76.892],[15.164,76.953],[15.148,76.958],[15.137,76.946],[15.122,76.959],[15.104,76.943],[15.109,76.897],[15.130,76.889]], {color:'#49aaff',fillColor:'#2686d8',fillOpacity:.17,weight:1,dashArray:'5 6',interactive:false});
  ...
  useEffect(() => {
    ...
    farms.forEach((farm,index) => {
      ...
      const polygon=L.polygon(points,{color:active?'#ffffff':colorFor(farm),fillColor:colorFor(farm),fillOpacity:farm.Potentially_Missed?.58:.34,weight:active?3:farm.Potentially_Missed?2.4:1.5,dashArray:farm.Potentially_Missed?'8 4':undefined});
...
export default function Home() {
  const [view,setView]=useState<'map'|'claims'|'analytics'|'alerts'>('map');
  ...
  const nav=[['map','Command map',MapIcon],['claims','Claims',FileText],['analytics','Analytics',BarChart3],['alerts','Alerts',Bell]] as const;
  const metrics=[['Farms analysed',counts.all,'Current view'],['Severely affected',counts.severe,'≥70% AI damage'],['Claims received',counts.claims,'Demo records'],['Potentially missed',counts.missed,'No corresponding claim'],['High priority',counts.high,'Field verification']];
  ...
  return <main className="shell">
    <header className="topbar">
      <button className="brand" onClick={()=>setView('map')}><span className="brand-mark"><Leaf size={19}/></span><span><strong>TerraAid</strong><small>Relief intelligence</small></span></button>
      ...
      <div className="source-pill"><i/> NASA GIBS · connected</div>
    </header>
    <section className="intro">
      <div><span className="eyebrow">SIH26206 · POST-FLOOD ASSESSMENT · {location.name.toUpperCase()}</span><h1>{view==='map'?'Find the farms the relief system may have missed.':view==='claims'?'Compare claims with satellite evidence.':view==='analytics'?'See what the evidence says at a glance.':'Turn evidence into field action.'}</h1><p>Satellite evidence, claim records and field priorities—one operational view.</p></div>
      <Dialog><DialogTrigger className="method"><Info size={14}/> Data &amp; method</DialogTrigger><DialogContent className="method-dialog"><DialogHeader><DialogTitle>Data &amp; method</DialogTitle><DialogDescription>Transparent by design. TerraAid supports authorised human decisions; it does not approve or reject claims.</DialogDescription></DialogHeader><div className="method-list"><div><Satellite/><span><b>Satellite imagery</b><small>NASA GIBS MODIS Terra True Color. Historical date requests are sent to the live WMS service.</small></span></div><div><Sparkles/><span><b>TerraAid analysis</b><small>May combine flood evidence, vegetation change and AI/change detection.</small></span></div><div><Database/><span><b>Prototype records</b><small>Claims and farm boundaries are synthetic demonstration data—not official cadastral parcels.</small></span></div><div><ShieldCheck/><span><b>Human authority</b><small>Field verification and final relief decisions remain with authorised officials.</small></span></div></div></DialogContent></Dialog>
    </section>
    {view==='map' && <>
      <section className="metric-grid">{metrics.map(([label,value,note],index)=><article className={`metric ${index===3?'critical':''}`} key={label}><span>{label}</span><strong>{value}</strong><small>{note}</small></article>)}</section>
      <section className="workspace">
        <aside className="filters">
          <div className="panel-heading"><span className="eyebrow">EVIDENCE FILTERS</span><button onClick={clearFilters}>Reset</button></div>
...
function FarmPanel({farm}:{farm:Farm}) { return <aside className={`priority-card ${farm.Potentially_Missed?'missed-card':''}`}>
  <span className="eyebrow purple-text">{farm.Potentially_Missed?'HIGHEST PRIORITY SIGNAL':'SELECTED FARM'}</span><div className="farm-title"><span>{farm.Farm_ID}</span><em className={`priority-${farm.Priority.toLowerCase()}`}>{farm.Priority}</em></div>
...
function ClaimsView({farms,search,onSearch,onOpen}:{farms:Farm[];search:string;onSearch:(v:string)=>void;onOpen:(f:Farm)=>void}) { return <section className="page-panel"><div className="section-head"><div><span className="eyebrow">CLAIM / EVIDENCE REVIEW</span><h2>Claims register</h2><p>Synthetic claim records compared with prototype satellite analysis.</p></div><div className="table-search"><Search/><input value={search} onChange={e=>onSearch(e.target.value)} placeholder="Search Farm_ID…"/></div></div><div className="table-wrap"><table><thead><tr><th>Farm ID</th><th>Crop</th><th>Claim</th><th>Claimed damage</th><th>AI damage</th><th>Mismatch</th><th>Priority</th><th/></tr></thead><tbody>{farms.map(f=><tr key={f.Farm_ID}><td><b>{f.Farm_ID}</b></td><td>{f.Crop}</td><td><span className={`table-status ${f.Claim_Submitted==='No'?'no':''}`}>{f.Claim_Submitted==='Yes'?'Submitted':'No claim'}</span></td><td>{f.Claim_Submitted==='Yes'?`${f.Claimed_Damage}%`:'—'}</td><td>{f.AI_Damage_Percent}%</td><td>{f.Evidence_Mismatch?<span className="mismatch-chip">Review</span>:'Aligned'}</td><td><span className={`priority-chip priority-${f.Priority.toLowerCase()}`}>{f.Priority}</span></td><td><button onClick={()=>onOpen(f)}>View on map →</button></td></tr>)}</tbody></table></div><div className="legal-note"><ShieldCheck/>Satellite evidence is decision support, not legal or automatic claim determination.</div></section>; }

function AnalyticsView({severityCounts,severeClaimed,severeTotal}:{severityCounts:{severe:number;moderate:number;low:number};severeClaimed:number;severeTotal:number}) { const total=severityCounts.severe+severityCounts.moderate+severityCounts.low; const missed=FARMS.filter(f=>f.Potentially_Missed).length; return <section className="page-panel analytics"><div className="section-head"><div><span className="eyebrow">DISTRICT EVIDENCE SUMMARY</span><h2>Damage intelligence</h2><p>Only decision-relevant indicators from the current synthetic assessment.</p></div></div><div className="analytics-grid"><article className="chart-card"><div><span>Damage severity</span><strong>{total} farms</strong></div><div className="donut" style={{background:`conic-gradient(#ff6b65 0 ${severityCounts.severe/total*100}%,#eeb84f 0 ${(severityCounts.severe+severityCounts.moderate)/total*100}%,#51d49a 0)`}}><span>{severityCounts.severe}<small>severe</small></span></div><ul><li><i className="red"/>Severe <b>{severityCounts.severe}</b></li><li><i className="amber"/>Moderate <b>{severityCounts.moderate}</b></li><li><i className="green"/>Low / minor <b>{severityCounts.low}</b></li></ul></article><article className="chart-card bars-card"><div><span>Claims among severe farms</span><strong>{severeTotal} severe cases</strong></div><div className="bar-item"><label><span>Claim submitted</span><b>{severeClaimed}</b></label><i><em style={{width:`${severeClaimed/severeTotal*100}%`}}/></i></div><div className="bar-item no-claim-bar"><label><span>No claim</span><b>{severeTotal-severeClaimed}</b></label><i><em style={{width:`${(severeTotal-severeClaimed)/severeTotal*100}%`}}/></i></div><small>Every unclaimed severe case is queued for human review.</small></article><article className="chart-card focus-card"><span className="eyebrow purple-text">CORE INNOVATION</span><strong>{missed}</strong><h3>Potentially missed beneficiaries</h3><p>Severe damage evidence + no corresponding claim.</p><div><AlertTriangle/>Field verification recommended</div></article></div></section>; }
